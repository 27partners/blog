...
class sspmod_27partners_Auth_Process_AzureGroups extends SimpleSAML_Auth_ProcessingFilter {
  ...
  public function process(&$request) {
    ...
    SimpleSAML_Logger::debug('Loading groups from Azure AD for user ' . $attributes['http://schemas.microsoft.com/identity/claims/objectidentifier'][0]);
    ...
    foreach ($groupmembership->body->value as $group) {
  ...
  private function checkresponse($response, $message) {
    ...
    throw new SimpleSAML_Error_Exception(sprintf('AzureGroups Error: "%s", http:%s, azure:"%s"',
      $message,
      $response->code,
      $azuremsg
    ));
